...
@onready var _submit_button: Button = $Panel/ScrollContainer/VBoxContainer/Buttons/SubmitButton
...
func fetch_scores() -> void:
	...
	if not _selected_boss_key.is_empty():
		query.eq("boss_type", _selected_boss_key)
	var result: Dictionary = await _run_database_query(db, query, &"selected")
	if not bool(result.get("success", false)) and not _selected_boss_key.is_empty():
		# Fallback for DB schemas that do not yet include a boss_type column.
		var fallback_query: SupabaseQuery = SupabaseQuery.new().from(TABLE_NAME).select().order("score", SupabaseQuery.Directions.Descending).range(0, 9)
		result = await _run_database_query(db, fallback_query, &"selected")
		if bool(result.get("success", false)):
			_loading_label.text = "Boss filter unavailable. Showing global leaderboard."
			_loading_label.visible = true
...
func submit_score(player_name: String, score: int, refresh_on_success: bool = true) -> bool:
	...
	if not _selected_boss_key.is_empty():
		insert_row["boss_type"] = _selected_boss_key
	var query: SupabaseQuery = SupabaseQuery.new().from(TABLE_NAME).insert([insert_row])
	var result: Dictionary = await _run_database_query(db, query, &"inserted")
	if not bool(result.get("success", false)) and not _selected_boss_key.is_empty():
		# Fallback insert for DB schemas without boss_type.
		var fallback_insert: SupabaseQuery = SupabaseQuery.new().from(TABLE_NAME).insert([{"player_name": safe_name, "score": score}])
		result = await _run_database_query(db, fallback_insert, &"inserted")
	if bool(result.get("success", false)):
		_name_status_label.text = ""
	...
	_name_status_label.text = "Submit failed: %s" % _extract_error_from_payload(result.get("payload"))
	return false
...
func _on_boss_filter_selected(index: int) -> void:
	_selected_boss_key = String(_boss_filter_option.get_item_metadata(index))
	_update_name_ui()
	fetch_scores()
...
func _update_name_ui() -> void:
	...
	if safe_name.is_empty():
		...
		_name_state_label.text = "Welcome, %s. Personal Score: %d" % [safe_name, personal_score]
...
func _populate_boss_filter() -> void:
	...
	_boss_filter_option.add_item("All Bosses")
	_boss_filter_option.set_item_metadata(0, "")
	...
	for definition in definitions:
		...
		if display_name.is_empty() or display_name == "???":
			display_name = "Boss %d" % level_id
		var boss_key: String = String((definition as Resource).get("boss_type")).strip_edges()
		if boss_key.is_empty():
			boss_key = "level_%d" % level_id
		_boss_filter_option.add_item(display_name)
		var item_index: int = _boss_filter_option.get_item_count() - 1
		_boss_filter_option.set_item_metadata(item_index, boss_key)

func _apply_scene_flow_context() -> void:
	var flow: Node = get_node_or_null("/root/SceneFlow")
	if flow == null or not flow.has_method("consume_leaderboard_context"):
		return
	var context: Dictionary = flow.call("consume_leaderboard_context")
	if context.is_empty():
		return

	_current_score = int(context.get("score", _current_score))
	if context.has("boss_type"):
		_selected_boss_key = String(context.get("boss_type", "")).strip_edges()
	elif context.has("level_id"):
		var level_id: int = int(context.get("level_id", 0))
		var config: Node = get_node_or_null("/root/GameConfig")
		if config != null and level_id > 0:
			var definition: Resource = config.call("get_level_definition", level_id)
			if definition != null:
				_selected_boss_key = String(definition.get("boss_type")).strip_edges()
		if _selected_boss_key.is_empty() and level_id > 0:
			_selected_boss_key = "level_%d" % level_id

	for i in range(_boss_filter_option.get_item_count()):
		if String(_boss_filter_option.get_item_metadata(i)) == _selected_boss_key:
			_boss_filter_option.select(i)
			break

func _handle_entry_submission() -> void:
	var auto_submitted: bool = false
	...
	var is_new_best: bool = bool(player_data.call("try_submit_score_for_boss", boss_key, _current_score))
	if not is_new_best:
		return

	var player_name: String = String(player_data.get("player_name")).strip_edges()
	if player_name.is_empty():
		_pending_auto_submit_score = _current_score
		_show_name_dialog("Enter a display name to submit your new personal best.")
		return

	auto_submitted = await submit_score(player_name, _current_score, false)
	if auto_submitted:
		_show_new_best_notification()
	_submit_button.visible = not auto_submitted

func _get_active_boss_key_for_personal_score() -> String:
	if not _selected_boss_key.is_empty():
		return _selected_boss_key
	return "global"
